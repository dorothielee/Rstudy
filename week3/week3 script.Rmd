...
여러 데이터 테이블을 총칭 하여 **관계형 데이터**라고 한다. 개별 데이터셋이 아니라 이들의 관계가 중요한 것이기 때문이다.
...
by = "key"를 통하여 조인을 하였지만, 다른 방법으로 데이터 테이블을 연결 할 수도 있다.

- 기본값 by = NULL로 할 경우, 모든 변수를 사용하며, 이를 natural join 이라고 부른다.
...
anti_join(x, y) : y와 매칭되는 x의 모든 관측값을 삭제한다.
